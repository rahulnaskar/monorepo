...
#### Step 1
**Initialize a `vite` app in `apps/frontend` folder of `root`. Following is the series of commands that will do the job for you**
...
**That's about everything which should enable us to controle our `monorepo` to be able to handle 2 independent projects**
...
Well, to do that first we need to install an `npm` package called `npm-run-all`. Enter the following command to do that:
...
In the above section when we were building the `backend` only, I was trying to demonstrate `npm`'s capability to build all `workspaces`. But that capability has run it's course for our purposes, hence the change.
