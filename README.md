...
**Add `utils` folder and set it up to house a simple function to add 2 numbers**

Here's the set of `commands` that does the thing (again)
...
First let's change the things in the default generated `package.json` and `tsconfig.json` to suit our needs.

> package.json

```json
{
  "name": "@repeatmono/utils",
  "version": "1.0.0",
  "description": "",
  "main": "dist/utils.js",
...
**Notice the name `@repeatmono/utils` that will be used by other modules as a dependency. Also there are changes in the `main` and `types` properties. Added `build` step in the script section to match with other packages**
...
**Note the changes in `outDir`, `target`, `verbatimeModuleSyntax`, `include` and `exclude`**

Add a function `addNumbers` in `utils.ts`. Attached a sample, please use your imagination instead.
...
From `packages/utils` folder run `build` to test independent working of this package.
...
**Build `packages/utils` from the `root` folder alongside others.**

But before that, we need to ensure that `utils` project can be built from root with a single command `build`. To do that switch to `root` folder and

1. Change `package.json` of `root` folder to include `packages` as in `workspaces`. The section should look something like this:
...
If you see 3 projects build successfully, you are on the clear.

#### Step 3

**Use the utility function `addNumbers` from `backend`**

Switch to `apps/backend` folder. Run the following command to install `packages/utils` as a dependency.

```bash
> npm install ../../packages/utils
```

It should modify `dependencies` section of `root` folder's `package.json` to something like this:

```json
  "dependencies": {
    "@repeatmono/utils": "file:../../packages/utils",
    "express": "^5.2.1"
  }
```

Make some changes in `apps/backend/src/main.ts` to use the function `addNumbers`. The entire file should look like:
...
should print the following in the last line of your console:

> Adding numbers from local package 7


</details>

### That's it in setting up a `monorepo`. As an exercise you can try using the function `addNumbers` from `apps/frontend`.
